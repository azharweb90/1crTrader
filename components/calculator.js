...
(function () {
  ...
  const tierRulesMatrix = {
    "small-1": { cap: 25000, pct: 7.2, loss: 1800 },
    "small-2": { cap: 50000, pct: 3.6, loss: 1800 },
    "small-3": { cap: 75000, pct: 2.4, loss: 1800 },
  ...
  function onLogDateInput() {
    ...
    const outOfRange =
      chosen && ((joinDate && chosen < joinDate) || (today && chosen > today));
  ...
  function updateYourLimitCallout() {
    ...
    if (maxLossEl) {
      maxLossEl.innerText = rule
        ? `Rs. ${fmt(rule.loss)} (${rule.pct.toFixed(2)}%)`
        : "—";
    }
  }
  ...
  function onCooldownComplete() {
    ...
    if (t2Status)
      t2Status.innerText =
        "Cooldown complete. Pick an outcome for this final trade.";
  ...
  function resetTracker(keepCooldown) {
    ...
    if (t1Instr) {
      t1Instr.value = "";
      t1Instr.disabled = false;
    }
    if (t2Instr) {
      t2Instr.value = "";
      t2Instr.disabled = true;
    }
    ...
    if (t1ProfitBtn) {
      t1ProfitBtn.disabled = false;
      t1ProfitBtn.className = "outcome-btn";
    }
    if (t1LossBtn) {
      t1LossBtn.disabled = false;
      t1LossBtn.className = "outcome-btn";
    }
    ...
    if (t2ProfitBtn) {
      t2ProfitBtn.disabled = true;
      t2ProfitBtn.className = "outcome-btn";
    }
    if (t2LossBtn) {
      t2LossBtn.disabled = true;
      t2LossBtn.className = "outcome-btn";
    }
    ...
    if (t2Status)
      t2Status.innerText =
        "Submit Trade 1 to see if Trade 2 becomes available.";
  ...
  function renderTradeState() {
    ...
    if (!trade1Submitted) {
      t1ProfitBtn.className =
        "outcome-btn" + (trade1Status === "profit" ? " active-profit" : "");
      t1LossBtn.className =
        "outcome-btn" + (trade1Status === "loss" ? " active-loss" : "");
    }
    if (trade2Unlocked && !trade2Submitted) {
      t2ProfitBtn.className =
        "outcome-btn" + (trade2Status === "profit" ? " active-profit" : "");
      t2LossBtn.className =
        "outcome-btn" + (trade2Status === "loss" ? " active-loss" : "");
    }
  ...
  function updateTradeSubmitButtons() {
    ...
    if (t1Btn && t1Status) {
      if (trade1Submitted) {
        ...
        t1Status.innerText =
          "Locked in and added to your balance and history below.";
      } else {
  ...
  function submitTrade1() {
    ...
    if (trade1Status === "profit") {
      t1RuleStatus = {
        compliant: true,
        label: "Profit \u2014 awaiting stop/continue decision",
        source: "manual",
      };
    } else if (trade1Amount >= rule.loss) {
      t1RuleStatus = {
        compliant: true,
        label: "Kill switch \u2014 100% of daily loss",
        source: "manual",
      };
    } else if (
      (rule.loss > 0 ? trade1Amount / rule.loss : 0) >= SOFT_BLOCK_RATIO
    ) {
      t1RuleStatus = {
        compliant: true,
        label: "Soft block \u2014 75%+ of daily loss",
        source: "manual",
      };
    } else {
      t1RuleStatus = {
        compliant: true,
        label: "Sub-cutoff loss \u2014 cooldown started",
        source: "manual",
      };
    }
    if (typeof window.recordCompletedDay === "function") {
      trade1TradeId = window.recordCompletedDay(
        netResult,
        chosenDate,
        trade1Instrument,
        t1RuleStatus,
  ...
  function chooseStopForToday() {
    ...
    if (t2Status)
      t2Status.innerText =
        "You chose to stop for today after Trade #1\u2019s profit. Trade #2 stays locked for the rest of the day.";
  ...
  function submitTrade2() {
    ...
    const t2RuleStatus = {
      compliant: true,
      label: "Final trade \u2014 day over after this",
      source: "manual",
    };
    if (typeof window.recordCompletedDay === "function") {
      trade2TradeId = window.recordCompletedDay(
        netResult,
        chosenDate,
        trade2Instrument,
        t2RuleStatus,
  ...
  function renderCalculatorHistory() {
    ...
    const history =
      typeof window.getTradeHistory === "function"
        ? window.getTradeHistory()
        : [];

    if (!history || history.length === 0) {
      container.innerHTML =
        '<div class="roadmap-empty-state">No trades submitted yet. Submit Trade 1 above to see it here.</div>';
      return;
    }
  ...
  function openQuickJournalModal(tradeNum) {
    ...
    if (tradeId) {
      ...
      if (statusEl)
        statusEl.innerText =
          "Draft note \u2014 saved automatically with this trade once you submit it.";
    }
  ...
  function saveQuickJournalNote() {
    ...
    if (quickJournalTradeId) {
      ...
      if (typeof window.saveJournalEntry === "function") {
        window.saveJournalEntry(quickJournalTradeId, {
          setupReason: strategy,
          emotion: emotion,
        });
      }
      if (statusEl)
        statusEl.innerText =
          "Saved. You can expand this into the full entry anytime from the Trading Journal tab.";
      ...
      if (statusEl)
        statusEl.innerText =
          "Draft saved \u2014 it will attach automatically when you submit this trade.";
    } else {
      return;
    }
  ...
  function commitJournalDraft(tradeNum, tradeId) {
    ...
    if (
      (draft.strategy || draft.emotion) &&
      typeof window.saveJournalEntry === "function"
    ) {
      window.saveJournalEntry(tradeId, {
        setupReason: draft.strategy,
  ...
  function renderBrokerRangeView() {
    ...
    const summary =
      typeof window.getBrokerPnlSummary === "function"
    ...
    if (realizedEl) {
      ...
      realizedEl.className =
        "broker-stat-value " +
        (summary.realizedPnl >= 0 ? "calc-history-win" : "calc-history-loss");
    }
  ...
  function renderBrokerMultiMonthGrid(from, to) {
    ...
    const pnlData =
      typeof window.getBrokerPnlHistory === "function"
        ? window.getBrokerPnlHistory()
        : {};
    ...
    months.forEach((monthDate) => {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (dateObj < from || dateObj > to) {
          dotsHtml +=
            '<span class="broker-month-dot broker-month-dot-empty"></span>';
          continue;
        }
        ...
        if (rows.length > 0) {
          dotClass =
            netForDay >= 0 ? "broker-month-dot-green" : "broker-month-dot-red";
          monthTotal += netForDay;
          hasAnyTrade = true;
        }

        const selectedClass =
          dateStr === brokerSelectedDate ? "broker-month-dot-selected" : "";
      ...
      const totalClass =
        monthTotal >= 0 ? "calc-history-win" : "calc-history-loss";
      const totalText = hasAnyTrade
  ...
  function showBrokerDayDetail(dateString) {
    ...
    const rows =
      typeof window.getBrokerPnlHistory === "function"
        ? window.getBrokerPnlHistory(dateString)
        : [];
    const importBtn = document.getElementById("broker-import-btn");

    if (rows.length === 0) {
      tableEl.innerHTML =
        '<div class="roadmap-empty-state">No trades reported by your broker on this day.</div>';
    ...
    let html =
      '<div class="broker-scrip-grid"><div class="broker-scrip-grid-head">Scrip</div><div class="broker-scrip-grid-head num">Qty</div><div class="broker-scrip-grid-head num">Buy</div><div class="broker-scrip-grid-head num">Sell</div><div class="broker-scrip-grid-head num">Charges</div><div class="broker-scrip-grid-head num">Net P&amp;L</div></div>';
    let dayTotal = 0;
    rows.forEach((r) => {
      dayTotal += r.netPnl;
    ...
    if (alreadyLogged) {
      tableEl.insertAdjacentHTML(
        "beforeend",
        '<p class="foot-note" style="margin-top:8px;">Heads up: this date already has trades in your Trade Log. Importing will add these as additional entries alongside them.</p>',
      );
    }
  }
  ...
  function requestImportBrokerDay() {
    ...
    const rows =
      typeof window.getBrokerPnlHistory === "function"
        ? window.getBrokerPnlHistory(brokerSelectedDate)
        : [];
  ...
  function evaluateBrokerTradeCompliance(
    netResult,
    tradesAlreadyThatDay,
    cooldownActive,
  ) {
    const tierKey = document.getElementById("calc-tier")
      ? document.getElementById("calc-tier").value
      : "small-1";
    const rule = tierRulesMatrix[tierKey] || tierRulesMatrix["small-1"];
    const isProfit = netResult > 0;
    const lossRatio =
      !isProfit && rule.loss > 0 ? Math.abs(netResult) / rule.loss : 0;
    ...
    if (tradesAlreadyThatDay >= 2) {
      return {
        ...
        message:
          "You\u2019d already hit your 2-trade limit for the day. Taking another trade anyway is overtrading, and it\u2019s exactly the pattern that erodes accounts over the long run. The edge isn\u2019t in this extra trade \u2014 it\u2019s in not taking it.",
        source: "broker",
      };
    }
    ...
    if (cooldownActive) {
      return {
        ...
        message:
          "This trade was taken before the 30-minute reset from the previous one finished. Re-entering this fast, win or lose, is usually emotion driving the trade, not a setup. Traders who survive long-term are the ones who can sit on their hands \u2014 protect that habit.",
        source: "broker",
      };
    }
  ...
  function confirmImportBrokerDay() {
    ...
    if (
      typeof window.getBrokerPnlHistory !== "function" ||
      typeof window.recordBrokerSyncedTrade !== "function"
    )
      return;
    ...
    rows.forEach((row) => {
      ...
      const compliance = evaluateBrokerTradeCompliance(
        row.netPnl,
        tradesSoFar,
        cooldownActive,
      );
      window.recordBrokerSyncedTrade(
        row.netPnl,
        row.instrumentLabel,
        compliance,
        dateString,
      ...
      const lossRatio =
        !isProfit && rule.loss > 0 ? Math.abs(row.netPnl) / rule.loss : 0;
